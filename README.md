# CUDA_LBU-v.2.0
Lost Btc unlocker tool for stalkers community and nVidia GPUs
...
All these addresses are baked into the CUDA_LBU_v2.exe tool and collectively hold around $300 billion.
...
1. Open the `CUDA_LBU_v2` folder and locate `CUDA_LBU_v2.exe` inside.
...
3. Add the `CUDA_LBU_v2` folder to your antivirus whitelist and to Windows Defender exclusions 
...
Choose CUDA_LBU_v2 folder`).

`CUDA_LBU_v2` works with GPUs supporting CUDA 10 and above.
...
Double-click `CUDA_LBU_v2.exe`. In the program window, You will see:
(example)
...
It shows the CUDA devices in your system.
...
4) From the aviable devices list enter the number of CUDA devices to use(Program will accept quantity counting from top to bottom of list):
**Quantity of CUDA devices: (e.g., 2)

5) **Quantity of GPU blocks [10..512]: (e.g., 191 for GPU)

6) **Number of threads per block [32..512], multiple of 32: (e.g., 192 for GPU)
...
[OK] Config saved in: ..\CUDA_LBU\config.json (on following program using you will able to use saved config on start)
...
Put it in the end of ..\CUDA_LBU\bin\points.dev (open with text editor and save)
...
Run the test CUDA_LBU with this ranges in Linear mode.
